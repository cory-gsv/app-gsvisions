...
export default function DeleteSiteButton({ siteId, label, className, bookingId, recipientName, recipientEmail, appointmentStart, bookingCancelled = false }: Props) {
  ...
  return <>
    <button className={className} type="button" onClick={() => { setOpen(true); setError(""); }} style={{ ...buttonStyle, borderColor: "rgba(164,61,50,.58)", color: "#a43d32", borderRadius: 0 }}>{bookingCancelled ? "Permanently Delete" : bookingId ? "Cancel Booking" : "Delete Test Site"}</button>
