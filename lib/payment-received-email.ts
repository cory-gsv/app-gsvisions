...
export async function sendPaymentReceivedEmail(args: {
  ...
  const methodLabel = paymentMethodLabel(args.paymentMethod || "stripe");
  ...
  const text = [`Hi ${firstName},`, "", `We received your payment of ${money(totalReceived, args.currency)} for ${address}.`, `Payment method: ${methodLabel}`, `Paid at: ${paidTime} PT`, `${statusHeading}: ${statusLabel}`, ...(actionLines.length ? ["", ...actionLines] : []), "", "Cory", "Golden State Visions · (916) 432-3373 · gsvisions.co"].join("\n");
