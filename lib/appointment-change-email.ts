...
export async function scheduleAppointmentChangeEmail(args: {
  ...
  const auditRecipient = clean(process.env.APPOINTMENT_CHANGE_BCC) || "cory@gsvisions.co";
  const auditBcc = recipient.toLowerCase() !== auditRecipient.toLowerCase() ? [auditRecipient] : undefined;
